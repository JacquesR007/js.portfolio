...
function placeOrder() {
    ...
    if (deliveryPrice === 0) {
        theOutput += "<p>You get free delivery ";

    }
    else {
        theOutput += "<p>Your total Delivery is: R " + (deliveryPrice);
    }


theOutput += "<p>Your total is: R <p/>" + (orderPrice + deliveryPrice);
// display the output
...
function calculateDelivery(orderPrice, deliveryCity) {
    ...
    if (( deliveryCity === "Edmonton") && (orderPrice > 240)){

        deliveryPrice = 0;}
else { deliveryCity = 35}
